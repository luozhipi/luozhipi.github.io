...
- `Human neck modeling`: Proposed a method for modeling and simulating the human neck and its skin deformation. Starting from the Ultimate Human Model (UHM) data set which includes a comprehensive anatomy structures of the human body, a model ready for the simulation of the neck's bio-mechanics and musculoskeletal dynamics is developed. The work involves interdisciplinary knowledge: first use Maya to deal with the geometric issues such as holes and degeneration of the polygonal meshes in order to facilitate the meshing procedure by Tetgen. The muscles represented as volumetric meshes resulting from meshing are modeld by the Finite Element Method with co-rotational material, bones as rigid bodies, and skin as a mass-spring network. Remarkably, proposed to use linear, elastic springs as soft constraints for coupling the rigid (bone) and soft (muscle) bodies. Skeleton motion is simulation by multi-body dynamic system. The overall dynamics is formulated as a Lagrangian dynamic system, which is numerically solved by semi-implicit time integrator. In order to obtain substaintial speed ups, considerable effort is made like reducing the number of finite elements, empirically selecting a subset of anatomy structures, and linearization.
